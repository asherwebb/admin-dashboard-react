var ProfileTextInputModule = React.createClass({
	...
	render: function(){
		var moduleState = this.state.underEditing ? 
			<div className="panel" >
				<input ref={this.props.objKey} type="text" defaultValue={this.props.data} />
				<br />
				<button className="btn btn-success" onClick={this.updateModule}>
					Save
				</button>
			</div>
			:
		...
		return(
			<div>
				...
				{moduleState}
				<button onClick={this.toggleEditTrigger} > 
...
var ProfileCheckboxInputModule = React.createClass({
	...
	render: function(){
		var isCheckedBtn = this.props.data ? 
			<button  type="button" className="btn btn-success" onClick={this.toggleCheck} key={this.props.objKey}>
				<span className="glyphicon glyphicon-ok-sign"></span>
			</button> 
			: 
			<button  type="button" className="btn btn-default faded" onClick={this.toggleCheck} key={this.props.objKey}>
				<span className="glyphicon glyphicon-ok-sign"></span>
			</button>;
		return (
			<div className="panel">
...
var ProfileTextAreaInputModule = React.createClass({
	...
	updateModule: function(e){ 
		e.preventDefault();
		var update = this.refs.about_hotel.getDOMNode().value, editElem = this.props.editElem, key = this.props.objKey;
		this.props.onUpdate( key, update, editElem );
		this.toggleEdit();
	},
	render: function(){
		var moduleState = this.state.underEditing ? 
			<div>
				<textarea ref="about_hotel" defaultValue={this.props.data}>
				</textarea>
				<br />
				<button className="btn btn-success" onClick={this.updateModule}>
					Save
...
var HotelImage = React.createClass({
	...
	updateModule: function(e) {
		var key = this.props.objKey, image;
	...
	render: function(){
		var moduleState = this.state.underEditing ? 
			<div>
				<input ref={this.props.objKey} type="file" />
				<br />
				<button className="btn btn-success" onClick={this.updateModule}>
					Save
				</button>
